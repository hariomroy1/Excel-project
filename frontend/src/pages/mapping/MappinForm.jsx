...
const FormExample = () => {
  ...
  const fetchColumnData = async () => {
    try {
      const response = await axiosInstance.get(`${EXCEL_API}/api/getColumnData`);
  ...
  //also provide automapped feature converted both column name to lowercase
  ...
  //     const requestData = selectedColumns.map(selectedColumnName => {
  //       // Find the corresponding columnData entry based on the selected column name in formData
  //       const matchingColumn = columnData.find(column => column.name === selectedColumnName);
  ...
  //     await axios.post(`${EXCEL_API}/api/SaveMappingFile`, requestData);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
     ...
     //-------------------------------------------------------------------------
       // Add unmapped columns with internalcolumn id 32 and type Additional
  //   const unmappedColumns = columnData
  //   .filter(({ name }) => !formData[name])
  //   .map(({ name }) => ({
  //     IngestionId: parseInt(ingestionId),
  //     usercolumn: name, // Use the column name as usercolumn
  //     internalcolumn: {
  //       id: 32,
  //       type: "Additional",
  //     },
  ...
  style={{
    ...
    marginLeft: "600px",
    borderRadius: "10px",
    background: isFormValid ? "#007bff" : "#cccccc", // Change background color based on validity
    color: isFormValid ? "#fff" : "#999999", // Change text color based on validity
    border: "none",
    cursor: isFormValid ? "pointer" : "not-allowed", // Change cursor style when disabled
  }}
>
  Submit
</button>
