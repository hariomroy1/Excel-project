  ...
  try {
    ...
    const result = await executeQuery(
      ...
      [
        ...
        `/public/temp/${excelFile.filename}`,
        "STORED_IN_TEMP_FOLDER",
      ]
    );
    ...
    //console.log(sheet)
    const columnNames = Object.keys(sheet).filter((key) =>
      /^[A-Z]+$/.test(key)
    ...
    res
      .status(200)
      .json({ message: "Excel file uploaded successfully", username });
  } catch (error) {
