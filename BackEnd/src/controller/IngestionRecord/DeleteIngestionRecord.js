...
//delete ingestoin record
const deleteIngestionRecord = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;
    //setup connections
    const connection = await getConnection();
    //check record is already there or not
    const existingRecord = await executeQuery(
      "SELECT * FROM IngestionRecords WHERE id = ?",
      [id]
    );
    ...
    await executeQuery(
      "DELETE FROM MappingColumns WHERE IngestionRecordId = ?",
      [id]
    );
    ...
    res
      .status(200)
      .json({
        message: "Ingestion record and associated files deleted successfully",
      });
  } catch (error) {
