...
// we got internal column header which i already inserted them into database
const InternalColumnHeader = asyncHandler(async (req, res) => {
  try {
    const connection = await getConnection();
    //fetch the internal column header
    const records = await executeQuery(
      "SELECT Id, Name,Type FROM InternalColumns ORDER BY Id ASC"
    );
