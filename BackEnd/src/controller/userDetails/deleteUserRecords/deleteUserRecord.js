...
// Function to update ingestion details by ID
const deleteUserRecords = asyncHandler(async (req, res) => {
    try {
        const { id } = req.params; // ingestionId from the path parameter
        const { dataId } = req.query; // dataId from the query parameter
        const deleteData = req.body; // Array of objects containing the data to delete
        ...
        // Delete the rows based on the provided deleteData array
        deleteData.forEach(deleteItem => {
            const { id: deleteId } = deleteItem;
            const rowIndex = parseInt(deleteId, 10) - 1; // Adjust index to match array index
            if (rowIndex >= 0 && rowIndex < rows.length) {
                rows.splice(rowIndex, 1); // Remove the row
            }
        });
        ...
        console.error("Error deleting details from Excel file:", error);
        res.status(500).json({ message: "Internal server error" });
    }
});


export { fetchIngestionDetailsById, deleteUserRecords };
